...
class Hero{
    ...
    public function __construct($id, $name, $class_id, $pv, $mana, $strength, $initiative, $shield, $xp, $current_level, $armor_id, $primary_wp_id, $secondary_wp_id, $weight_limit, $qte_item_limit){
        ...
        $this->armor = $armor_id;
        $this->primary_wp = $primary_wp_id;
        $this->secondary_wp = $secondary_wp_id;
        $this->weight_limit = $weight_limit;
        $this->qte_item_limit = $qte_item_limit;
    }

    public function getArmor(){
        return Armor::getArmor($this->armor);
    }

    public function getPrimary(){
        return Weapon::getWeapon($this->primary_wp);
    }

    public function getSecondary(){
        return Weapon::getWeapon($this->secondary_wp);
    }
    ...
    public function getTotalWeight():Float{
        ...
        if(isset($this->armor)){
            $sum += $this->getArmor()->getTotalWeight();
        }
        if(isset($this->primary_wp)){
            $sum += $this->getPrimary()->getTotalWeight();
        }
        if(isset($this->secondary_wp)){
            $sum += $this->getSecondary()->getTotalWeight();
        }
        return $sum;
    }
